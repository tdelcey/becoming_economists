...
thesis_metadata <- readRDS(here(FR_cleaned_data_path, "thesis_metadata.RDS")) %>% ungroup
...
thesis_metadata %>% 
  ...
  filter(these_id %in% c(these_id[[7]], duplicates[[7]])) %>% 
  DT::datatable(
    rownames = FALSE,
    options = list(
      pageLength = 5,
