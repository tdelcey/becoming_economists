...
language_fastText <- identify_fastText_language(
  ...
  id_obj = thesis_metadata[! is.na(title_other)]$these_id,
  pre_trained_language_model_path = file_ftz) %>% 
  as.data.table

# Merge detected languages into `thesis_metadata`
thesis_metadata <- merge(thesis_metadata, language_fastText, by = "these_id", all.x = TRUE) 
...
language_abstract_fastText <- identify_fastText_language(input_obj = thesis_metadata[! is.na(abstract_other)]$abstract_other, # import from helper_functions.R
                                                         id_obj = thesis_metadata[! is.na(abstract_other)]$these_id,
                                                         pre_trained_language_model_path = file_ftz) %>% 
  as.data.table

thesis_metadata <- merge(thesis_metadata, language_abstract_fastText, by = "these_id", all.x = TRUE)
...
language_fastText_fr <- future_map2(thesis_metadata[!is.na(title_fr)]$title_fr,
                                    thesis_metadata[!is.na(title_fr)]$these_id,
  ...
  rbindlist
setnames(language_fastText_fr, c("language_fastText_fr", "prob_fastText_fr", "these_id"))

language_fastText_en <- future_map2(thesis_metadata[!is.na(title_en)]$title_en,
                                    thesis_metadata[!is.na(title_en)]$these_id,
                                    ~ identify_fastText_language(input_obj = .x, # import from helper_functions.R
                                                                 id_obj = .y,
                                                                 pre_trained_language_model_path = file_ftz,
                                                                 verbose = FALSE) %>% 
  ...
  rbindlist
setnames(language_fastText_en, c("language_fastText_en", "prob_fastText_en", "these_id"))

# Merge language detections into the main table
thesis_metadata <- merge(thesis_metadata, language_fastText_fr, by = "these_id", all.x = TRUE)
thesis_metadata <- merge(thesis_metadata, language_fastText_en, by = "these_id", all.x = TRUE)
...
language_fastText_abstract_fr <- future_map2(thesis_metadata[!is.na(abstract_fr)]$abstract_fr,
                                             thesis_metadata[!is.na(abstract_fr)]$these_id,
                                             ~ identify_fastText_language(input_obj = .x, # import from helper_functions.R
                                                                          id_obj = .y,
                                                                          pre_trained_language_model_path = file_ftz,
                                                                          verbose = FALSE) %>% 
  ...
  rbindlist
setnames(language_fastText_abstract_fr, c("language_fastText_abstract_fr", "prob_fastText_abstract_fr", "these_id"))

language_fastText_abstract_en <- future_map2(thesis_metadata[!is.na(abstract_en)]$abstract_en,
                                             thesis_metadata[!is.na(abstract_en)]$these_id,
                                             ~ identify_fastText_language(input_obj = .x, # import from helper_functions.R
                                                                          id_obj = .y,
                                                                          pre_trained_language_model_path = file_ftz,
                                                                          verbose = FALSE) %>% 
  ...
  rbindlist
setnames(language_fastText_abstract_en, c("language_fastText_abstract_en", "prob_fastText_abstract_en", "these_id"))

# Merge language detection into the main table
thesis_metadata <- merge(thesis_metadata, language_fastText_abstract_fr, by = "these_id", all.x = TRUE)
thesis_metadata <- merge(thesis_metadata, language_fastText_abstract_en, by = "these_id", all.x = TRUE)
...
data_to_match <- thesis_edge[entity_role == "author",][thesis_metadata, on = "these_id"]
setorder(data_to_match, these_id, entity_id)
...
data_to_match <- data_to_match[!is.na(title) & title != "" & !is.na(authors) & authors != "" , .(these_id, title, authors)] 
...
duplicates <- duplicates[, .(duplicates = list(these_id_2)), by = "these_id"]

# Add duplicates to the main metadata table
thesis_metadata <- thesis_metadata %>%
  left_join(duplicates[, .(these_id, duplicates)]) %>% 
...
specific_cases <- thesis_metadata %>%
  arrange(these_id) %>%
  matching_duplicate_manually(these_id, pattern = "Approche systémique et régulation économique") %>%
  select(contains("id")) %>%
  group_by(id_new) %>%
  mutate(duplicates = list(these_id),
         duplicates = map(duplicates, ~.[-1])) %>% # remove the first element of the list, the id of the line 
  ungroup %>%
  select(these_id, duplicates) %>%
  unique

# Add specific cases to metadata
thesis_metadata <- thesis_metadata %>%
  mutate(duplicates = ifelse(these_id %in% specific_cases$these_id, specific_cases$duplicates, duplicates))
