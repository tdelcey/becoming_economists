...
duplicates <- duplicates[, .(duplicates = list(c(thesis_id, thesis_id_2))), by = "thesis_id"]

# sort the list of duplicates
duplicates <- duplicates[, duplicates := map(duplicates, ~ sort(.x))]
...
thesis_metadata[title_fr %like% "Approche systémique et régulation économique", duplicates := map(thesis_id, ~ c(.x, specific_cases$thesis_id))]
# sort 
...
test_dt <- tidyfast::dt_unnest(test_dt, duplicates, keep = FALSE) %>% 
