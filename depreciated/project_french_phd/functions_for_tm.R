...
create_stm <- function(data, min_word_number = 200){ 
  ...
  #removing data sets with two few vocabularies (if vocab inferior to the number of topics, the algorithm through an error)
...
name_topics <- function(data, 
  ...
  labels <- data %>% 
    ...
    mutate(topic = paste0("Topic ", id),
           topic_name = paste0(topic, ": ", term_label)) %>% 
    unique()
}
